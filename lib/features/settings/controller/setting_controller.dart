...
class SettingController extends GetxController {
  ...
  void _purgeAllControllers() {
    // 1. Registered 여부 확인 후 'force' 옵션으로 확실히 제거 🧹
  ...
  Future<void> _reinitializeCoreSystems() async {
    // 2. [Crucial] Add 'permanent: true' to ensure instances survive the navigation transition
    Get.put(TabbarController(), permanent: true);
    await loadSettings(); 

    // 3. Re-establish Dependency Injection Graph with new database connection
    Get.put(DashboardHandler(), permanent: true); 
    
    // final dash is needed for the immediate refresh call below
    final dash = Get.put(DashboardController(), permanent: true); 
    Get.put(CategoryController(), permanent: true);
    Get.put(CalendarController(), permanent: true);

    // 4. Update data based on the newly connected database
